<template>
  <div>
    <div class="search">
      <el-input placeholder="请输入模板名称查询" style="width: 200px" v-model="name"></el-input>
    ...
    <div class="table">
      <el-table :data="tableData" stripe  @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="name" label="模板名称" width="80" align="center" sortable></el-table-column>
        <el-table-column prop="categoryName" label="计划表类别" show-overflow-tooltip></el-table-column>
        <el-table-column prop="creationTime" label="创建时间" show-overflow-tooltip></el-table-column>
        <el-table-column prop="user" label="所属用户" show-overflow-tooltip></el-table-column>
        <el-table-column prop="eventCount" label="事件数量" show-overflow-tooltip></el-table-column>
    ...
    <el-dialog title="计划模板" :visible.sync="fromVisible" width="40%" :close-on-click-modal="false" destroy-on-close>
      <el-form label-width="100px" style="padding-right: 50px" :model="form" :rules="rules" ref="formRef">
        <el-form-item prop="name" label="模板名称">
          <el-input v-model="form.name" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item prop="categoryId" label="计划模板类别">
...
export default {
  ...
  methods: {
    loadGategory(){
      this.$request.get('templateCategory/selectAll').then(res=>{
        if(res.code == '200'){
    ...
    save() {   // 保存按钮触发的逻辑  它会触发新增或者更新
      this.$refs.formRef.validate((valid) => {
        if (valid) {
          this.$request({
            url: this.form.id ? '/template/update' : '/template/add',
    ...
    delBatch() {   // 批量删除
      ...
      this.$confirm('您确定批量删除这些数据吗？', '确认删除', {type: "warning"}).then(response => {
        this.$request.delete('/template/delete/batch', {data: this.ids}).then(res => {
          if (res.code === '200') {   // 表示操作成功
            this.$message.success('操作成功')
            this.load(1)
          } else {
    ...
    load(pageNum) {  // 分页查询
      ...
      this.$request.get('/template/selectPage', {
        params: {
